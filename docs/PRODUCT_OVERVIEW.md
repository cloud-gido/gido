...
| **GIDO Stream**（玑渡·流） | 实时流计算与数据管道 | Flink SQL / JAR 开发、Kafka→Paimon 配置化管道、状态化运维、发布审批 |
...
![GIDO 登录页](screenshots/01-login.png)

![GIDO 产品选择](screenshots/02-product-selector.png)
...
![GIDO Batch 数据开发](screenshots/03-batch-studio.png)
...
![GIDO Stream 作业开发](screenshots/04-stream-studio.png)

**核心菜单**

| 模块 | 说明 |
|------|------|
| 作业开发 | Flink SQL / JAR 编辑、草稿与版本历史、提交不可变发布版本；提交不直接启动作业 |
| 数据管道 | Kafka→Paimon 向导、Schema Contract、字段映射、生成物预览与发布前预检 |
| 作业运维 | SQL/JAR/Pipeline 发布版本部署、Savepoint 停止、选择恢复点重启、资源调优与诊断 |
| 资源管理 | JAR、连接器与依赖文件版本管理；与作业运维同级 |
| 发布审批 | 普通开发提交的流作业发布版本审批；批准后由运维显式部署 |
...
![GIDO Serve 服务概览](screenshots/05-serve-overview.png)
...
| GIDO 主题与外观 | GIDO 关于页 |
|:---:|:---:|
| ![GIDO 主题与外观](screenshots/06-theme-appearance.png) | ![GIDO 关于页](screenshots/07-about.png) |
