...
6. 点击默认「保存并停止」（默认等待 Snapshot **约 300s**），确认过渡状态依次包含「正在保存状态 / 正在挂起」，最后为已停止；运维页进度应提示正在等待 Snapshot。
7. 确认：
   ...
   - 恢复点历史存在成功 Savepoint 路径（与 Snapshot/路径一致），且可看到执行耗时；
...
11. **热重启对齐**：作业仍在运行、尚无成功恢复点时，运维页「重启/恢复」应允许「热重启（同配置，无需恢复点）」，与后端 `restartNonce` 路径一致；不得强制只能无状态启动。
12. **同步竞态**：重启/部署回收重建 CR 的短暂窗口内，列表不得闪成「已停止」（lifecycle 应保持 `RESTORING`/`DEPLOYING`）。
...
- 没有成功恢复点且作业**未在运行**时，「最近 / 指定 Savepoint」不可选，但无状态启动仍可按权限显式执行；作业**仍在运行**时可热重启（无需恢复点）。
- 操作历史中清理集群应显示中文「清理集群」（操作类型 `force-stop`），不得裸露英文 key。
