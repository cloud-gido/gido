...
describe('editor session + script keybindings adoption', () => {
  ...
  it('Studio / Stream 使用 editorSessionStore；Probe / NodeConfigModal 不叠 Tab session', () => {
    ...
    expect(studio).toContain('seedSessionTabs')
    expect(studio).toContain('ensureTabContent')
    expect(studio).toContain('isEditorTabContentPending')
    expect(studio).toContain('activateTab')
    expect(studio).toContain('tabContentError')
    expect(studio).toContain('StudioEditorTabStrip')
    expect(studio).toContain('planStudioSessionTabOrder')
    expect(studio).not.toContain('const hydrateTabs')
