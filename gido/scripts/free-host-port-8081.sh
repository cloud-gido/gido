...
echo ""
echo "=== Kubernetes：flink 命名空间（遗留 k8s/legacy/flink.yaml 中 JM Service 为 LoadBalancer）==="
...
if command -v kubectl >/dev/null 2>&1; then
  if kubectl get ns flink >/dev/null 2>&1; then
    ...
    echo "      要腾出 8081 给本机 docker-compose Flink：可 kubectl delete -f <仓库根>/k8s/legacy/flink.yaml，"
...
echo ""
echo "=== 本机 kubectl port-forward（若曾跑 k8s/legacy/port-forward-jobmanager.sh）==="
...
echo ""
echo "提示：8081 冲突时可用本脚本排查；K8s JM 占用时可 kubectl delete -f k8s/legacy/flink.yaml 或改 Service 类型。"
echo "     遗留 K8s 清单见 k8s/legacy/；生产流作业见 k8s/deploy-gido-k3s.sh。"
