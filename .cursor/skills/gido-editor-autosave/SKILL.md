...
- **SQL 库表补全**：`monacoSqlCompletion` + `sqlSchemaCache` + `useSqlSchemaCompletion`；Studio / Probe 的 SQL Monaco 须同步接线。后端 `integration` tables/columns 支持可选 `catalog`，另有 `/schemas`。库表抽屉 `SqlSchemaBrowserDrawer` 共用缓存。手测见 `docs/STUDIO_SQL_COMPLETION.md`。
