...
4. Flink 运行时镜像：GitHub `dev`/`main` push 后 CI 自动推 GHCR `gido-flink-runtime`（见 `k8s/flink-sql-runner/README.md`、`.github/workflows/ci.yml`）；自建 ECR 时：`k8s/build-flink-runtime.sh`
